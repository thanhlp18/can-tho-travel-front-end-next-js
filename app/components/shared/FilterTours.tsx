...
export default function FilterTours() {
  ...
  function handleSearch(e: any) {
    ...
    const tags = selectedTag.join(",");
    if (searchQuery || selectedTag.length > 0) {
      router.push(
        `/tours?title=${searchQuery ? searchQuery : ""}&category=${tags}
        `
      );
    } else {
      router.push("/tours");
  ...
  useEffect(() => {
    setAllTagLabels([
      ...JSON.parse(localStorage.getItem("allTourTagLabels") ?? "[]"),
    ]);
  }, []);
  ...
  return (
    <section aria-labelledby="top-post">
      <div className="w-full text-center">
        <h2
          id="top-post"
          className="text-center text-2xl font-extrabold uppercase text-tertiary inline-block px-2 mb-10 max-md:mb-4"
        >
          FILTER
        </h2>
      </div>
